...
function getTrackThumbnail(info) {
  ...
  if (!thumbnail && info.uri?.includes('youtu.be')) {
    const videoId = info.uri.split('youtu.be/')[1]?.split('?')[0];
    if (videoId) thumbnail = `https://img.youtube.com/vi/${videoId}/maxresdefault.jpg`;
  }

  return thumbnail || 'https://i.imgur.com/QYJfXQv.png';
}
  ...
  return new ContainerBuilder()
    ...
    .addActionRowComponents(
      new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId('back').setEmoji('<:left:1538544846459899955>').setStyle(ButtonStyle.Secondary).setDisabled(disabled),
        new ButtonBuilder().setCustomId(isPaused ? 'resume' : 'pause').setEmoji(isPaused ? '<:play:1538541584167997503>' : '<:pause:1538541612353855489>').setStyle(ButtonStyle.Secondary).setDisabled(disabled),
        new ButtonBuilder().setCustomId('skip').setEmoji('<:right:1538541644695998545>').setStyle(ButtonStyle.Secondary).setDisabled(disabled),
        new ButtonBuilder().setCustomId('queue').setEmoji('<:folder:1538542808648908891>').setStyle(ButtonStyle.Secondary).setDisabled(disabled),
        new ButtonBuilder().setCustomId('stop').setEmoji('<:cancel:1538544866659672144>').setStyle(ButtonStyle.Danger).setDisabled(disabled)
      )
    );
}
...
function createQueueContainer(player) {
  ...
  if (current?.info) {
    description += `**Now Playing:**\n**[${current.info.title}](${current.info.uri})**\n` +
      `${current.info.author || 'Unknown'} • ${formatTime(current.info.length || 0)} • <@${current.info.requester || '0'}>\n\n`;
  }
  ...
  if (queue.length > 0) {
    ...
    queue.slice(0, 10).forEach((track, index) => {
      const info = track.info || {};
      description += `\`${index + 1}.\` **[${info.title || 'Unknown'}](${info.uri || 'https://youtube.com'})**\n` +
        `${info.author || 'Unknown'} • ${formatTime(info.length || 0)} • <@${info.requester || '0'}>\n`;
    });
...
riffy.on('trackStart', async (player, track) => {
  ...
  if (!trackHistory.has(guildId)) trackHistory.set(guildId, []);

  const history = trackHistory.get(guildId);
  const lastTrack = lastPlayedTracks.get(guildId);
  const isNavigation = navigationActions.has(guildId);

  if (lastTrack && lastTrack.info?.uri !== track.info?.uri && !isNavigation) {
...
riffy.on('queueEnd', async (player) => {
  ...
  trackHistory.delete(player.guildId);
  lastPlayedTracks.delete(player.guildId);
  navigationActions.delete(player.guildId);
  player.destroy();
});
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (!player) {
    return interaction.reply({ content: `${config.emojis.error} No player found`, flags: MessageFlags.Ephemeral });
  }

  const member = interaction.member;
  if (!member.voice?.channel) {
    return interaction.reply({ content: `${config.emojis.error} You need to be in a voice channel`, flags: MessageFlags.Ephemeral });
  }

  if (member.voice.channel.id !== player.voiceChannel) {
    return interaction.reply({ content: `${config.emojis.error} You need to be in the same voice channel`, flags: MessageFlags.Ephemeral });
  }

  switch (interaction.customId) {
    case 'back': {
      const history = trackHistory.get(player.guildId) || [];

      if (history.length > 0) {
        player.queue.unshift(history.pop());
        navigationActions.add(player.guildId);
        player.stop();
        return interaction.reply({ content: 'Back', flags: MessageFlags.Ephemeral });
      }

      if (!player.current) {
        return interaction.reply({ content: `${config.emojis.error} Nothing is playing`, flags: MessageFlags.Ephemeral });
      }

      player.queue.unshift(player.current);
      navigationActions.add(player.guildId);
      player.stop();
      return interaction.reply({ content: 'Restarted', flags: MessageFlags.Ephemeral });
    }
    ...
    case 'resume': {
      ...
      return interaction.reply({ content: shouldPause ? 'Paused' : 'Resumed', flags: MessageFlags.Ephemeral });
    }
    ...
    case 'skip': {
      ...
      player.stop();
      return interaction.reply({ content: 'Next', flags: MessageFlags.Ephemeral });
    }
...
if (config.enablePrefix) {
  client.on('messageCreate', async (message) => {
    ...
    if (!command || !['play', 'p'].includes(command)) return;
    ...
    if (!isLavalinkConnected) return message.reply(`${config.emojis.error} Lavalink is not connected. Music commands are unavailable.`);
    ...
    try {
      ...
      return message.reply(`${config.emojis.error} An error occurred while playing the song`);
    }
  });
}
...
client.once('ready', async () => {
  ...
  try {
    ...
    console.log(`${config.emojis.success} Riffy initialized`);
  } catch (error) {
    console.error(`${config.emojis.error} Failed to initialize Riffy:`, error);
  }

  console.log(`${config.emojis.success} Prefix command system ready`);
});
